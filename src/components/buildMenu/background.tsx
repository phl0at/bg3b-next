...
import BackgroundsObj from "@/lib//character/backgrounds";
import Image from "next/image";
import { Background } from "@/lib/types";

const BackgroundComponent = () => {
  const { dispatch }: { dispatch: ({ }) => void } = useStore((state) => state);
  const { origin, background }: { origin: number; background: number } = useStore(state => state.current)
  const BackgroundsArr = Object.values(BackgroundsObj)
  ...
  return (
    <>
      <div className="grid grid-cols-4 gap-x-3 gap-y-5 w-[90%] text-amber-100">
        {(BackgroundsArr).slice(0, BackgroundsArr.length - 1).map((element: Background) => {
          return (
            <div
      ...
      <div className="w-[80%] gap-y-3 flex flex-col mt-12 text-left text-amber-100">
        ...
        <p className="text-md">{BackgroundsObj[background as keyof typeof BackgroundsObj].description}</p>
      </div>
    </>
  );
};
