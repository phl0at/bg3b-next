...
const ClassComponent = () => {
  ...
  const handleClick = (_class: StaticClass) =>
      ...
      {display === "class" && (
        <>
          <div className="grid grid-cols-4 gap-x-3 gap-y-5 w-[90%] text-amber-100">
            {Object.values(Classes).map((element: StaticClass) => {
              return (
                <div
          ...
          <div className="w-[80%] gap-y-3 flex flex-col mt-12 text-left text-amber-100">
            ...
            <p className="text-md">{Classes[selectedClass as keyof typeof Classes].description}</p>
          </div>
        </>
      )}
      {display === "subclass" && (
